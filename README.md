...
Kredensial Baileys disimpan secara terenkripsi pada PostgreSQL/Supabase dalam tabel `WhatsAppSession`, bukan pada filesystem container. Setelah satu kali scan QR/kode pairing, sesi dimuat kembali otomatis ketika backend Abasthan diredeploy atau dihidupkan ulang. Sesi hanya dihapus oleh tombol **Putuskan Sesi** (atau jika WhatsApp sendiri membatalkan perangkat tertaut).

Backend bot tetap harus berupa Web Service Node.js yang aktif; frontend boleh di Vercel. Tidak diperlukan volume filesystem persistent Abasthan untuk menyimpan sesi WhatsApp.
