...
export const handleIncomingWhatsAppMessage = async (req: Request, res: Response) => {
  try {
    ...
    const senderPhone = String(from).trim();
    ...
    if (upperText === 'RESET' || upperText === 'BATAL') {
      ...
      return res.status(200).json({
        status: 'success',
        message: 'Percakapan berhasil direset dari awal.',
        data: {
    ...
    // Save citizen message to history
    ...
    // STEP 4: DETAIL -> PROMPT FIRST MANDATORY PHOTO
      ...
      botReply = `Langkah 4 dari 4 (Wajib Lampirkan Dokumen Persyaratan):\n\n` +
        ...
        `\n\n📸 *Kirim Foto Ke-1:* Silakan kirim *${requiredList[0]}* terlebih dahulu menggunakan ikon Kamera 📷 atau Lampiran 📎 di bawah:`;
    } 
    // STEP 5: ASK_PHOTO -> HANDLES MULTIPLE PHOTOS SEQUENTIALLY
      ...
      // Check if more photos are still required
      if (session.photos.length < requiredList.length) {
        ...
        botReply = `*${expectedTitle} Berhasil Diterima!* ✓\n\n` +
          ...
          `📸 *Silakan kirim Foto Ke-${session.photos.length + 1}: ${nextRequired}* (Klik Kamera 📷 / Lampiran 📎 di bawah):`;
      } else {
        // ALL REQUIRED PHOTOS COMPLETE -> UNLOCKS CONFIRMATION STEP!
    ...
    // STEP 6: CONFIRMATION -> SUBMIT TO OPERATOR
      ...
      if (upperText === 'SETUJU' || upperText === 'YA' || upperText === 'OK') {
        ...
        // Store in shared memory with attached photos
        ...
        // Try persisting to PostgreSQL DB
        try {
